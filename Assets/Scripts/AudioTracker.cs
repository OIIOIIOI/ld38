...
public class AudioTracker : MonoBehaviour {
	...
	public void SoundBoard (int roomnumberint = 0, int puzzlenumberint = -1, int SFXnumberint = 0)
    {
        ...
        if (roomnumberint != currentroom) {
			...
			RoomBoard (currentroom);

		}

		if (puzzlenumberint != currentpuzzle) {
			StopAllCoroutines ();
	...
	/*void OnGUI (){
		...
		GUILayout.Box ("ROOM");
		roomnumberint = GUILayout.SelectionGrid (roomnumberint, Roommlist,4);

		GUILayout.Box ("PUZZLE");
		puzzlenumberint = GUILayout.SelectionGrid (puzzlenumberint, Puzzlelist,4);

		GUILayout.Box ("SFX");
		SFXnumberint = GUILayout.SelectionGrid (SFXnumberint, SFXlist,4);
	...
	IEnumerator FadeOut(AudioSource AudioCurrent, float aTime) {
		...
		//print ("OUT ok" + AudioCurrent.volume);
	}
